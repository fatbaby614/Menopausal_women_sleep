...
DATA_DIR = (r"E:\datasets\Sleep\sleep-edf-database-expanded-1.0.0" if IS_WINDOWS
...
def compute_all():
    ...
    results_dir = os.path.join(sys_path, 'experiment_results')
    ...
    for group_key, config in REPRESENTATIVE_SUBJECTS.items():
        ...
        # Use sleep window epochs for stage percentage calculation
        sleep_window_epochs = y[first_sleep:last_sleep + 1]
        sleep_window_n = len(sleep_window_epochs)
        sleep_window_pcts = {
            name: float(np.sum(sleep_window_epochs == i) / sleep_window_n * 100)
        ...
        all_results[group_key] = {
            ...
            'accuracy': accuracy,
            'stage_pct': sleep_window_pcts,
        }

        print(f"  OK - Accuracy: {accuracy*100:.1f}%")
        print(f"     Sleep Window ({sleep_window_n} epochs): "
              f"W={sleep_window_pcts['Wake']:.1f}% "
              f"N1={sleep_window_pcts['N1']:.1f}% "
              f"N2={sleep_window_pcts['N2']:.1f}% "
              f"N3={sleep_window_pcts['N3']:.1f}% "
              f"REM={sleep_window_pcts['REM']:.1f}%")
