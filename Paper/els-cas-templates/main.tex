...
Results: Across all three independent datasets, menopausal women consistently showed significantly reduced N3 deep sleep (6.8\% in Sleep-EDF, 17.1\% in ISRUC, 7.0\% in DREAMS) compared to young women (16.7\%, 19.5\%, 8.8\%, respectively) and increased N1 light sleep (13.1\%, 15.0\%, 8.9\% vs. 8.7\%, 7.3\%, 7.8\%). Wake-to-N1 transitions were markedly elevated, demonstrating 2.5-fold higher sleep state instability. In the comprehensive 6-algorithm SOTA benchmark, MenoSCA-FBTS achieved 87.8\% accuracy (ranking 1st overall), followed by YASA (87.0\%), Random Forest (86.7\%), TinySleepNet (85.4\%), EEGNet (78.5\%), and MiniRocket (72.0\%). The algorithm demonstrated robust performance across all three datasets.

Conclusion: This triple independent database cross-validation confirms with strong evidence that menopause is associated with profound, consistent sleep architecture alterations characterized by reduced deep sleep and increased sleep state instability. The comprehensive SOTA benchmark demonstrates that Riemannian geometry-based approaches (MenoSCA-FBTS, 87.8\%) achieve state-of-the-art performance on menopausal women's sleep staging, with YASA (87.0\%) and Random Forest (86.7\%) also performing competitively, providing multiple viable pathways for clinical deployment.
\end{abstract}
