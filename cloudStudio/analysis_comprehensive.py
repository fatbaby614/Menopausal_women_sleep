...
def compute_sleep_parameters(labels):
    ...
    tst = (last_sleep - first_sleep + 1) * EPOCH_LENGTH_S / 60
    tib = total_epochs * EPOCH_LENGTH_S / 60
    
    wake_in_sleep = np.sum(labels[first_sleep:last_sleep + 1] == 0)
    waso = wake_in_sleep * EPOCH_LENGTH_S / 60
    ...
    sleep_eff = (tst - waso) / tib * 100 if tib > 0 else 0
...
def analysis_transition_spectrum():
    ...
    for group_key, group_info in groups.items():
        ...
        for subj_id in group_info['subjects']:
            ...
            trans_metrics = compute_transition_metrics(epochs_labels)
...
def analysis_hypnodensity_quantitative():
    """Analysis 4: Hypnodensity quantitative analysis"""
    print("\n" + "=" * 70)
    print("Analysis 4: Hypnodensity Quantitative Analysis")
    print("=" * 70)
    
    results_file = 'results_pytorch/representative_results.json'
    ...
    for group_key, result in results.items():
        ...
        segments = {
            'First Hour': slice(0, min(epoch_per_hour, n_epochs)),
            'Middle Hour': slice(n_epochs // 2 - epoch_per_hour // 2, 
                           min(n_epochs // 2 + epoch_per_hour // 2, n_epochs)),
            'Last Hour': slice(max(0, n_epochs - epoch_per_hour), n_epochs)
        }
    ...
    print("\n[N3 Probability by Time Segment]")
    for seg_name in ['First Hour', 'Middle Hour', 'Last Hour']:
        ...
        for group_key in hypnodensity_results.keys():
            if seg_name in hypnodensity_results[group_key]:
                ...
                print(f"  {group_names.get(group_key, group_key)}: {n3_prob:.1f}%")
    
    print("\n[Mean All-Night Wake Probability]")
    for group_key, result in results.items():
        y_prob = np.array(result['y_prob'])
        wake_prob = np.mean(y_prob[:, 0]) * 100
        group_names = {'menopausal_women': 'Menopausal', 'young_women': 'Young W', 'young_men': 'Young M'}
...
def analysis_sleep_correlation():
    ...
    r_n3_eff, p_n3_eff = stats.pearsonr(n3_pcts, sleep_effs)
    r_n1_waso, p_n1_waso = stats.pearsonr(n1_pcts, wasos)
    r_n1_eff, p_n1_eff = stats.pearsonr(n1_pcts, sleep_effs)
