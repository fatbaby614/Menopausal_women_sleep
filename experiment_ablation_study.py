...
def load_menopausal_subjects():
    ...
    # Fallback to original list if CSV not found
...
def run_ablation_experiment(name, config):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(logo.split(X, y, groups)):
        ...
        print(f"    Subject {subj_in_test[0]} (Fold {fold_idx+1}/{n_subjects}): Acc={acc:.4f}, F1={f1:.4f}, Time={fold_time:.1f}s")
